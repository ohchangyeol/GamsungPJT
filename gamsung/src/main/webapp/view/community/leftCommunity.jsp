    ...
    <form role="form">            
      <div class="form-group">
        <select class="form-control" name="searchCondition">
          ...
          <option value="2" ${ ! empty search.searchCondition && search.searchCondition==0 ? "selected" : "" }>번호</option>
          <option value="3" ${ ! empty search.searchCondition && search.searchCondition==0 ? "selected" : "" }>내용</option>
          <option value="4" ${ ! empty search.searchCondition && search.searchCondition==0 ? "selected" : "" }>해시태그</option>
          			
        </select>
      </div> 				            
      <div class="search-box">
        <label class="sr-only" for="searchKeyword">검색어</label>
        <input type="text" class="form-control" id="searchKeyword" name="searchKeyword" placeholder="검색어를 입력해주세요." value="${! empty search.searchKeyword ? search.searchKeyword : '' }">
